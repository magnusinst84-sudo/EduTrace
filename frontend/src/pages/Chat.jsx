...
import { BookOpen, RotateCcw, Send, LogOut, X, ChevronLeft, ChevronRight, Home, MessageSquare, Map, User } from "lucide-react";
...
// ---------------------------------------------------------------------------
// Normalise the shape Gemini returns into what RoadmapWeekCard expects.
// Gemini:          { week, title, topics, resources, goal }
// RoadmapWeekCard: { week, topic, concepts, resources, hours, status }
// ---------------------------------------------------------------------------
        ...
        {roadmap.map((week, index) => (
          <div
            ...
            style={{
              ...
              animation: `fadeSlideIn 300ms ease-out ${index * 150}ms both`
            }}
          >
            <RoadmapWeekCard
              {...week}
  ...
  return (
    <>
      ...
      <div
        ...
        className="fixed bottom-0 left-0 right-0 z-50 bg-white rounded-t-2xl overflow-hidden shadow-2xl"
        style={{ height: '85vh' }}
      >
        {/* Handle bar */}
...
export default function Chat() {
  ...
  const sessionId = location.state?.session_id
    || sessionStorage.getItem('edutrace_session_id')
  const topic = location.state?.topic
    || sessionStorage.getItem('edutrace_topic')
    || 'React'
  ...
  const [messages, setMessages] = useState([
    {
      role: 'agent',
      content: `Hi! I'm EduTrace. Let's build your personalized ${topic} roadmap. First — what do you already know about ${topic}?`,
      timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
    },
  ])
  const [input,        setInput]        = useState('')
  const [typing,       setTyping]       = useState(false)
  const [roadmap,      setRoadmap]      = useState(null)   // normalised weeks array | null
      ...
      try {
        ...
        if (session.conversation_history?.length > 0) {
          const restored = session.conversation_history
            ...
            .map(e => ({
              role:      e.role === 'assistant' ? 'agent' : e.role,
              content:   e.content,
              timestamp: '',
            }))
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (sessionState === 'diagnostic') {
        res = await api.post(`/api/session/${sessionId}/diagnostic`,
        ...
        if (res.data.status === 'complete') {
          ...
          setMessages(prev => [...prev, {
            role:      'agent',
            content:   `Got it — you're at **${res.data.level}** level. Type "generate roadmap" when you're ready to see your personalized plan.`,
            timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
          }])
        } else {
          setMessages(prev => [...prev, {
      ...
      // ── Phase 2: generate roadmap ────────────────────────────────────────
        ...
        sessionState === 'roadmap_ready' &&
        (userMsg.content.toLowerCase().includes('generate') ||
         userMsg.content.toLowerCase().includes('roadmap')  ||
         userMsg.content.toLowerCase().includes('show'))
      ) {
        ...
        try {
          ...
          setWorldState(prev => prev
            ? { ...prev, totalWeeks: res.data.total_weeks ?? normalized.length ?? 8 }
            : null)
          setMessages(prev => [...prev, {
            role:      'agent',
            content:   `Your ${topic} roadmap is ready! It's ${res.data.total_weeks} weeks long. Check the panel on the right. What would you like to start with?`,
            timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
          }])
        } finally {
      ...
      // ── Phase 3: adaptive tutoring ───────────────────────────────────────
      } else {
        res = await api.post(`/api/session/${sessionId}/chat`,
  ...
  return (
    <div className="flex h-screen bg-gray-50 overflow-hidden" style={{ fontFamily: "Inter, sans-serif" }}>
      ...
      {/* Chat column */}
      <div
        className="flex flex-col min-w-0 transition-all duration-400 ease-out relative"
        style={{ flex: panelOpen && !isMobile ? '0 0 60%' : '1 1 100%' }}
      >
        {/* Top nav */}
        <header style={{ height: 52, backgroundColor: "#fff", borderBottom: "1px solid #E5E7EB", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 20px", flexShrink: 0 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <button
              ...
              style={{ padding: "5px 12px", borderRadius: 7, border: "1.5px solid #E5E7EB", backgroundColor: "#fff", fontSize: 12, color: "#EF4444", cursor: "pointer", display: "flex", alignItems: "center", gap: 5 }}
            >
        ...
        {/* Body containing messages and World State if visible */}
        <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
                ...
                <div className="flex flex-col gap-1">
                  ...
                  {generatingRoadmap && (
                    <p className="text-xs text-gray-400 px-2">
                      Generating your personalized roadmap, this may take 
                      up to 30 seconds...
                    </p>
                  )}
                </div>
            ...
            <div style={{ padding: "12px 16px 80px 16px", borderTop: "1px solid #E5E7EB", backgroundColor: "#fff", display: "flex", gap: 10, alignItems: "flex-end" }}>
              ...
              <button
                ...
                style={{
                  ...
                  height: 40,
                  borderRadius: 10,
                  backgroundColor: (input.trim() && !typing && !generatingRoadmap) ? "#4F46E5" : "#E5E7EB",
                  border: "none",
          ...
          {/* World State Panel overlay / sidebar if on desktop and roadmap is NOT open (or alongside it) */}
          {!isMobile && worldState && (
      ...
      {/* Desktop panel toggle button on divider */}
      {!isMobile && roadmap?.length > 0 && (
        <button
          onClick={() => setPanelOpen(p => !p)}
          className="flex-shrink-0 w-5 self-stretch flex items-center 
                     justify-center bg-gray-100 hover:bg-gray-200 
                     border-x border-gray-200 transition-colors z-10"
      ...
      {isMobile && (
        ...
        >
          {/* Display World State at the top of the bottom sheet, followed by the roadmap */}
          {worldState && (
